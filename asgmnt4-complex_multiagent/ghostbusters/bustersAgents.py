    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        ghostExpectedPositions = []
        for aBelief in livingGhostPositionDistributions:
            maxProbability = 0.0
            for key, value in aBelief.items():
                if value > maxProbability:
                    maxProbability = value
                    aPosition = key
            ghostExpectedPositions.append(aPosition)
        minDistance = float("inf")
        for anAction in legal: 
            tempMinDistance = float("inf")
            for aMaxPosition in ghostExpectedPositions:
                distance = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, anAction), aMaxPosition)
